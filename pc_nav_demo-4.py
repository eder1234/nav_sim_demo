    ...
    while not env.episode_over:
        ...
        if keystroke == ord('p'):
            ...
            if args.save_matched_points:
                matched_img, _, _, _ = visualize_matches(current_color, kp1, target_color, kp2, matches)
            ...
            if icp_result.fitness < 0.1:  # Adjust the fitness threshold as needed
                print("ICP registration failed.")
                # Take appropriate action or notify the user
            else:
                # Determine action based on the transformation matrix
                action_ = determine_bot_action(icp_result.transformation)
                print(f'Action: {action_}')
                # Optionally update the GUI or take further steps based on the action
